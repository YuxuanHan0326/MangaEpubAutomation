...
param(
    ...
    [ValidateSet('ignore', 'chapter_no_merge', 'chapter_merge')]

    [string]$ExtraGroupHandlingMode = 'ignore',
...
$GroupTankobon = ([char]0x5355).ToString() + ([char]0x884C) + ([char]0x672C) # å•è¡Œæœ¬

$GroupDefault = ([char]0x9ED8).ToString() + ([char]0x8A8D) # é»˜èª

$MetadataJsonName = ([char]0x5143).ToString() + ([char]0x6570) + ([char]0x636E) + '.json' # å…ƒæ•°æ®.json

$ChapterMetadataJsonName = ([char]0x7AE0).ToString() + ([char]0x8282) + ([char]0x5143) + ([char]0x6570) + ([char]0x636E) + '.json' # ç« èŠ‚å…ƒæ•°æ®.json
...
function Build-KccBaseArgsFromConfig {
    ...
    if (($KccConfig.PSObject.Properties.Name -contains 'BaseArgs') -and (@($KccConfig.BaseArgs).Count -gt 0)) {
...
function Resolve-PipelineConfig {
    ...
    $cfg.kcc.EffectiveArgsSource = if ((@($cfg.kcc.BaseArgs).Count -gt 0)) { 'kcc.BaseArgs' } else { 'kcc.CliOptions' }
...
function Get-WorkflowRef {
    ...
    if ($workflowValues.Count -lt 1) { throw 'Invalid config: Workflows.$values is empty' }
...
function Get-ComicMetadata {
    ...
    try {
        ...
        if ($meta -and ($meta.PSObject.Properties.Name -contains 'comic') -and $meta.comic) {
            ...
            if (($comic.PSObject.Properties.Name -contains 'author') -and ((@($comic.author)).Count -gt 0)) {
...
# Discover source groups to process for per-chapter EPUB packaging.

# Includes default groups plus any extra first-level folders.

function Get-EpubProcessingGroupNames {
    ...
    $names.Add($TankobonGroupName) | Out-Null

    $names.Add($DefaultGroupName) | Out-Null
    ...
    $existing = @(Get-ChildItem -LiteralPath $SourceRoot -Directory -ErrorAction SilentlyContinue)
    ...
    foreach ($dir in $existing) {
        ...
        if ($name -eq $TankobonGroupName -or $name -eq $DefaultGroupName) { continue }
...
# Fallback parser for chapter names like â€œç¬¬60.1è¯/è©±â€.
...
# Detect names that look like tankobon volumes (e.g. 第1卷 / 1巻 / Vol1卷).

function Test-IsVolumeLikeChapterName {

    param([Parameter(Mandatory = $true)][string]$ChapterName)



    return [regex]::IsMatch($ChapterName, '^\s*(?:\u7B2C\s*)?\d+(?:\.\d+)?\s*[\u5377\u5DFB]\s*$')

}



# Classify chapter folder as tankobon/chapter using group context + name rules.

# Rule summary:

# 1) Under å•è¡Œæœ¬ => always tankobon.

# 2) Non-å•è¡Œæœ¬ folders with special keywords => chapter.

# 3) Non-å•è¡Œæœ¬ folders matching strict volume pattern => tankobon.

# 4) Otherwise => chapter.

function Get-ChapterPackagingGroup {
    ...
    if ($SourceGroupName -eq $TankobonGroupName) {

        return $TankobonGroupName

    }



    $specialChapterPattern = '(?:\u8FDE\u8F7D\u7248|\u9023\u8F09\u7248|\u9644\u8D60|\u9644\u8D08|\u7EAA\u5FF5|\u7D00\u5FF5|\u7279\u5178|\u756A\u5916|\u7279\u522B|\u7279\u5225|\bSP\b)'

    if ([regex]::IsMatch($ChapterName, $specialChapterPattern, [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)) {
    ...
    if (Test-IsVolumeLikeChapterName -ChapterName $ChapterName) {
...
function Get-MergedEpubPlan {
    ...
    # anthology filename from order range (or fallback to "åˆé›†").
    ...
    $orderMap = Get-DefaultChapterOrderMap -SourceRoot $SourceRoot -DefaultGroupName $SourceGroupName -MetadataFileName $MetadataFileName -ChapterMetadataFileName $ChapterMetadataFileName

    $mergedNamePattern = if ([string]::IsNullOrWhiteSpace($TargetLabelOverride)) {

        '^{0}\s*-\s*{1}.+$' -f [regex]::Escape($ComicTitle), [regex]::Escape($TalkLabel)

    }

    else {

        '^{0}\s*-\s*{1}$' -f [regex]::Escape($ComicTitle), [regex]::Escape($TargetLabelOverride)
    ...
    # Defensive normalization: avoid strict-mode crashes if malformed items leak in.

    $chapterCandidates = @($chapterCandidates | Where-Object {

            $null -ne $_ -and

            ($_.PSObject.Properties.Name -contains 'ChapterName') -and

            ($_.PSObject.Properties.Name -contains 'EpubPath')

        } | ForEach-Object {

            [pscustomobject]@{

                ChapterName = [string]$_.ChapterName

                EpubPath = [string]$_.EpubPath

                HasOrder = [bool](($_.PSObject.Properties.Name -contains 'HasOrder') -and $_.HasOrder)

                Order = if ($_.PSObject.Properties.Name -contains 'Order') { $_.Order } else { $null }

                FileLength = if ($_.PSObject.Properties.Name -contains 'FileLength') { [int64]$_.FileLength } else { 0 }

                LastWriteTicks = if ($_.PSObject.Properties.Name -contains 'LastWriteTicks') { [int64]$_.LastWriteTicks } else { 0 }
    ...
    # Second-pass normalization protects strict mode when external/order overrides leak unexpected shapes.

    $sorted = @($sorted | ForEach-Object {

            if ($null -eq $_) { return }

            $name = if ($_.PSObject.Properties.Name -contains 'ChapterName') { [string]$_.ChapterName } else { [string]$_ }

            $epub = if ($_.PSObject.Properties.Name -contains 'EpubPath') { [string]$_.EpubPath } else { '' }

            $hasOrd = [bool](($_.PSObject.Properties.Name -contains 'HasOrder') -and $_.HasOrder)

            $ordVal = if ($_.PSObject.Properties.Name -contains 'Order') { $_.Order } else { $null }

            $flen = if ($_.PSObject.Properties.Name -contains 'FileLength') { [int64]$_.FileLength } else { 0 }

            $ticks = if ($_.PSObject.Properties.Name -contains 'LastWriteTicks') { [int64]$_.LastWriteTicks } else { 0 }

            if ([string]::IsNullOrWhiteSpace($name) -or [string]::IsNullOrWhiteSpace($epub)) { return }
    ...
    $orderedOnly = @($sorted | Where-Object { ($_.PSObject.Properties.Name -contains 'HasOrder') -and $_.HasOrder })



    # Prefer range naming based on chapter names (first/last numeric "xx?")

    # so tail specials like "???" do not force an incorrect numeric end label.

    $namedTalkNumbers = @()

    foreach ($ch in @($sorted)) {

        $chName = if ($ch -and ($ch.PSObject.Properties.Name -contains 'ChapterName')) { [string]$ch.ChapterName } else { '' }

        if ([string]::IsNullOrWhiteSpace($chName)) { continue }

        $numFromName = TryParse-ChapterOrderFromName -ChapterName $chName

        if ($null -ne $numFromName) { $namedTalkNumbers += [double]$numFromName }

    }



    if ([string]::IsNullOrWhiteSpace($TargetLabelOverride)) {

        if ($namedTalkNumbers.Count -gt 0) {

            $startText = Format-OrderValue -Value ([double]$namedTalkNumbers[0])

            $endText = Format-OrderValue -Value ([double]$namedTalkNumbers[$namedTalkNumbers.Count - 1])

            $rangeLabel = if ($startText -eq $endText) { "{0}{1}" -f $TalkLabel, $startText } else { "{0}{1}-{2}" -f $TalkLabel, $startText, $endText }

        }

        elseif ($orderedOnly.Count -gt 0) {
        ...
        $rangeLabel = $TargetLabelOverride
...
function Get-MergeManifestPath {
    ...
    param(
        ...
        [Parameter(Mandatory = $true)][string]$DefaultManifestFileName

    )



    if ($GroupName -eq $DefaultGroupName) {

        return (Join-Path $OutputRoot $DefaultManifestFileName)

    }



    $safeGroup = [regex]::Replace($GroupName, '[\\/:*?"<>|]', '_')

    return (Join-Path $OutputRoot (".manga_epub_automation_merge_manifest.{0}.json" -f $safeGroup))
...
function Estimate-EpubPackWork {
    ...
    $groups.Add($TankobonGroupName) | Out-Null

    $groups.Add($DefaultGroupName) | Out-Null

    foreach ($groupName in @($AdditionalGroupNames)) {

        if ([string]::IsNullOrWhiteSpace($groupName)) { continue }

        if (-not $groups.Contains($groupName)) { $groups.Add($groupName) | Out-Null }
    ...
    foreach ($group in $groups) {
        ...
        foreach ($chapterDir in $chapterDirs) {
            ...
            $effectiveGroup = Get-ChapterPackagingGroup -SourceGroupName $group -ChapterName $chapterDir.Name -TankobonGroupName $TankobonGroupName



            $effectiveTargetGroupPath = if ($effectiveGroup -eq $TankobonGroupName) { $PublishedOutputRoot } else { $sourceGroupPath }

            $epubName = Get-EpubFileName -GroupName $effectiveGroup -ChapterDirName $chapterDir.Name -ComicTitle $ComicTitle -TankobonGroupName $TankobonGroupName -VolumeLabel $VolumeLabel

            $targetPath = Join-Path $effectiveTargetGroupPath $epubName
...
$epubGroupNames = if ($null -ne $sourceDir) {

    if ($ExtraGroupHandlingMode -eq 'ignore') {

        @($GroupTankobon, $GroupDefault)

    }

    else {

        @(Get-EpubProcessingGroupNames -SourceRoot $sourceDir.FullName -TankobonGroupName $GroupTankobon -DefaultGroupName $GroupDefault)
...
$epubAdditionalGroupNames = @($epubGroupNames | Where-Object { $_ -ne $GroupTankobon -and $_ -ne $GroupDefault })

if ($ExtraGroupHandlingMode -eq 'chapter_merge') {

    $mergeExtraGroups = @($epubAdditionalGroupNames)
...
$epubEstimate = if ($runEpubStage -and $null -ne $sourceDir) {
    ...
    Estimate-EpubPackWork -UpscaledRoot $outputDir -PublishedOutputRoot $epubOutputDir -ComicTitle $comicMeta.ComicTitle -TankobonGroupName $GroupTankobon -DefaultGroupName $GroupDefault -VolumeLabel $VolumeChar -AdditionalGroupNames $epubAdditionalGroupNames
...
$planSummary = [pscustomobject]@{
    ...
    Modes = [pscustomobject]@{
        ...
        ExtraGroupHandlingMode = [string]$ExtraGroupHandlingMode
...
Write-Host ("ExtraGroups:  mode={0} epub_groups={1}" -f $ExtraGroupHandlingMode, (($epubGroupNames -join ', ')))
...
if ((-not $runUpscaleStage -or $backendExitCode -eq 0) -and $runEpubStage) {
    ...
    $epubStageEstimate = Estimate-EpubPackWork -UpscaledRoot $outputDir -PublishedOutputRoot $epubOutputDir -ComicTitle $comicMeta.ComicTitle -TankobonGroupName $GroupTankobon -DefaultGroupName $GroupDefault -VolumeLabel $VolumeChar -AdditionalGroupNames $epubAdditionalGroupNames
    ...
    foreach ($group in $groupNames) {
        ...
        foreach ($chapterDir in $chapterDirs) {
            ...
            $effectiveGroup = Get-ChapterPackagingGroup -SourceGroupName $group -ChapterName $chapterDir.Name -TankobonGroupName $GroupTankobon



            $effectiveTargetGroupPath = if ($effectiveGroup -eq $GroupTankobon) { $epubOutputDir } else { $sourceGroupPath }

            New-Item -ItemType Directory -Force -Path $effectiveTargetGroupPath | Out-Null

            $epubName = Get-EpubFileName -GroupName $effectiveGroup -ChapterDirName $chapterDir.Name -ComicTitle $comicMeta.ComicTitle -TankobonGroupName $GroupTankobon -VolumeLabel $VolumeChar
...
if ((-not $runUpscaleStage -or $backendExitCode -eq 0) -and $runMergeStage) {
    ...
    if (@($mergeExtraGroups).Count -gt 0) {

        foreach ($extraGroup in $mergeExtraGroups) {

            $extraGroupPath = Join-Path $outputDir $extraGroup

            if (-not (Test-Path -LiteralPath $extraGroupPath -PathType Container)) { continue }



            $extraPlan = Get-MergedEpubPlan -SourceRoot $sourceDir.FullName -UpscaledDefaultGroupPath $extraGroupPath -MergedOutputDefaultGroupPath $epubOutputDefaultGroupPath -ComicTitle $comicMeta.ComicTitle -SourceGroupName $extraGroup -MetadataFileName $MetadataJsonName -ChapterMetadataFileName $ChapterMetadataJsonName -TalkLabel $TalkChar -TargetLabelOverride $extraGroup

            $extraManifestPath = Get-MergeManifestPath -OutputRoot $epubOutputDefaultGroupPath -GroupName $extraGroup -DefaultGroupName $GroupDefault -DefaultManifestFileName $mergeManifestFileName

            $extraDecision = if ($extraPlan.Chapters.Count -gt 0) {

                Get-MergedRebuildDecision -ManifestPath $extraManifestPath -ManifestHash $extraPlan.ManifestHash -TargetPath $extraPlan.TargetPath -MergedFiles $extraPlan.MergedFiles
            ...
            if ($extraPlan.Chapters.Count -eq 0) {

                Write-StageMessage -Message ("MERGED_SKIP: no chapter EPUB candidates in group {0}." -f $extraGroup) -LogPath $logPath
            ...
            if ([string]::IsNullOrWhiteSpace($mergedEpubPath)) {

                $mergedEpubPath = $extraPlan.TargetPath

            }

            else {

                $mergedEpubPath = "{0}; {1}" -f $mergedEpubPath, $extraPlan.TargetPath

            }



            if (-not $extraDecision.NeedRebuild) {

                $mergedEpubSkipped += 1

                Write-StageMessage -Message ("MERGED_SKIP: target={0} reason={1}" -f $extraPlan.TargetPath, $extraDecision.Reason) -LogPath $logPath
            ...
            Write-StageMessage -Message ("MERGED_PACK: target={0} reason={1}" -f $extraPlan.TargetPath, $extraDecision.Reason) -LogPath $logPath



            $extraMergeTitle = [System.IO.Path]::GetFileNameWithoutExtension($extraPlan.TargetFileName)

            $extraMergeAuthor = if ([string]::IsNullOrWhiteSpace($MergedEpubAuthorFallback)) { $comicMeta.Author } else { $MergedEpubAuthorFallback }

            $extraDescLines = @([string]$pipelineConfig.merge.DescriptionHeader)

            foreach ($ch in $extraPlan.Chapters) {

                if ([bool]$pipelineConfig.merge.IncludeOrderInDescription -and $ch.HasOrder) {

                    $extraDescLines += ("{0} (order {1})" -f $ch.ChapterName, (Format-OrderValue -Value ([double]$ch.Order)))

                }

                else {

                    $extraDescLines += $ch.ChapterName
            ...
            $extraMergePlanJsonPath = Join-Path $logsDir ("merge_plan_{0}_{1}.json" -f $timestamp, ([regex]::Replace($extraGroup, '[\\/:*?"<>| ]', '_')))

            $extraMergePlanObject = [pscustomobject]@{

                output_epub_path = $extraPlan.TargetPath

                title = $extraMergeTitle

                author = $extraMergeAuthor

                language = [string]$pipelineConfig.merge.Language

                description = ($extraDescLines -join "`n")

                contributor = [string]$pipelineConfig.merge.MetadataContributor

                chapters = @($extraPlan.Chapters | ForEach-Object {

                        if ($_.HasOrder) { [pscustomobject]@{ chapter_name = $_.ChapterName; order = [double]$_.Order; epub_path = $_.EpubPath } }

                        else { [pscustomobject]@{ chapter_name = $_.ChapterName; order = $null; epub_path = $_.EpubPath } }

                    })

            }

            Write-JsonUtf8NoBom -Object $extraMergePlanObject -Path $extraMergePlanJsonPath



            $extraMergeCode = Invoke-MergedEpubPack -PythonExe $pythonExe -MergeScriptPath $mergeScriptPath -PlanPath $extraMergePlanJsonPath -LogPath $logPath

            if ($extraMergeCode -ne 0) {

                $mergedEpubFailed += 1

                Write-StageMessage -Message ("MERGED_FAIL: exit={0} target={1}" -f $extraMergeCode, $extraPlan.TargetPath) -LogPath $logPath -Warning

                continue

            }

            if (-not (Test-Path -LiteralPath $extraPlan.TargetPath -PathType Leaf)) {

                $mergedEpubFailed += 1

                Write-StageMessage -Message ("MERGED_FAIL: merge succeeded but target missing: {0}" -f $extraPlan.TargetPath) -LogPath $logPath -Warning
            ...
            $extraDeleteFailed = $false

            foreach ($oldFile in $extraDecision.OtherMerged) {

                try {

                    Remove-Item -LiteralPath $oldFile.FullName -Force

                    Write-StageMessage -Message ("MERGED_DELETE_OLD: {0}" -f $oldFile.FullName) -LogPath $logPath -DebugOnly

                }

                catch {

                    $extraDeleteFailed = $true

                    Write-StageMessage -Message ("MERGED_DELETE_FAIL: {0} ; {1}" -f $oldFile.FullName, $_.Exception.Message) -LogPath $logPath -Warning
            ...
            $extraManifestObject = [pscustomobject]@{

                ManifestHash = $extraPlan.ManifestHash

                MergeOrderFilePath = $null

                MergeOrderSignature = 'AUTO'

                MergedFileName = $extraPlan.TargetFileName

                MergedFilePath = $extraPlan.TargetPath

                ChapterCount = $extraPlan.Chapters.Count
            ...
            Write-JsonUtf8NoBom -Object $extraManifestObject -Path $extraManifestPath
